...
bool Pair::write(Op& op) {
  // std::cout<<"Pair::write 1"<<std::endl;
  if (state_ == CLOSED) {
    // std::cout<<"Pair::write 2"<<std::endl;
    return false;
  }
  // std::cout<<"Pair::write 3"<<std::endl;
  NonOwningPtr<UnboundBuffer> buf;
  std::array<struct iovec, 2> iov;
  int ioc;
  ssize_t rv;
  // std::cout<<"Pair::write 4"<<std::endl;
  ...
  for (;;) {
    ...
    if (rv == -1) {
      ...
      if (errno == ECONNRESET) {
        // std::cout<<"Pair::write 11"<<std::endl;
        if (!sync_) {
          // std::cout<<"Pair::write 12"<<std::endl;
          return false;
        }
      }
      if (errno == EPIPE) {
        // std::cout<<"Pair::write 13"<<std::endl;
        if (!sync_) {
          // std::cout<<"Pair::write 14"<<std::endl;
          return false;
        }
      }
      ...
      // Retry on EINTR
      if (errno == EINTR) {
        // std::cout<<"Pair::write 15"<<std::endl;
        continue;
      }
      // std::cout<<"Pair::write 16"<<std::endl;
      // Unexpected error
      signalException(
...
void Pair::writeComplete(const Op &op, NonOwningPtr<UnboundBuffer> &buf,
  ...
  // std::cout<<"Pair::writeComplete start"<<std::endl;
  switch (opcode) {
    case Op::SEND_BUFFER:
      // std::cout<<"Pair::writeComplete 1"<<std::endl;
      op.buf->handleSendCompletion();
      break;
    case Op::SEND_UNBOUND_BUFFER:
      // std::cout<<"Pair::writeComplete 2"<<std::endl;
      buf->handleSendCompletion(this->rank_);
      break;
    case Op::NOTIFY_SEND_READY:
      // std::cout<<"Pair::writeComplete 3"<<std::endl;
      break;
    case Op::NOTIFY_RECV_READY:
      // std::cout<<"Pair::writeComplete 4"<<std::endl;
      break;
  }
  // std::cout<<"Pair::writeComplete end"<<std::endl;
}
...
//
bool Pair::read() {
  // std::cout<<"transport:tcp:read() 1"<<std::endl;
  if (state_ == CLOSED) {
    return false;
  }
  NonOwningPtr<UnboundBuffer> buf;
  ...
  for (;;) {
	  // std::cout<<"transport:tcp:read()"<<std::endl;
    struct iovec iov = {
        .iov_base = nullptr,
        .iov_len = 0,
    };
      ...
      if (rv == -1) {
        ...
        if (errno == EAGAIN) {
          ...
          if (sync_) {
            ...
            // std::cout<<"transport:tcp:read2() 6"<<std::endl;
        ...
        // Retry on EINTR
        if (errno == EINTR) {
          // std::cout<<"transport:tcp:read2() 7"<<std::endl;
    ...
    // std::cout<<"transport:tcp:read2() 8"<<std::endl;
      
    // Transition to CLOSED on EOF
    if (rv == 0) {
      // std::cout<<"transport:tcp:read2() 9"<<std::endl;
      
      signalException(
          GLOO_ERROR_MSG("Connection closed by peer ", peer_.str()));
          // std::cout<<"transport:tcp:read2() 10"<<std::endl;
      return false;
    }
  ...
  readComplete(buf);
  // std::cout<<"transport:tcp:readComplete"<<std::endl;
  return true;
}
...
void Pair::handleRemotePendingRecv(const Op& op) {
  // std::cout<<"Pair::handleRemotePendingRecv start"<<std::endl;
  const auto& slot = op.preamble.slot;
  ...
  // std::cout<<"Pair::handleRemotePendingRecv end"<<std::endl;
}
...
void Pair::sendAsyncMode(Op& op) {
  ...
  if (write(op)) {
    // std::cout<<"Pair:sendAsyncMode 3"<<std::endl;
    return;
  }
  // std::cout<<"Pair:sendAsyncMode 4"<<std::endl;
  // Write may have resulted in an error.
  throwIfException();
  // std::cout<<"Pair:sendAsyncMode 5"<<std::endl;
  // Write didn't complete; pass to event loop
  tx_.push_back(std::move(op));
  // std::cout<<"Pair:sendAsyncMode 6"<<std::endl;
  device_->registerDescriptor(fd_, EPOLLIN | EPOLLOUT, this);
  // std::cout<<"Pair:sendAsyncMode 7"<<std::endl;
}
...
void Pair::send(
    ...
    size_t offset,
    size_t nbytes) {
      // std::cout<<"Pair: send start: "<<this<<std::endl;
  ...
  if (mutator.shiftRemotePendingRecv()) {
    ...
    // std::cout<<"Pair: send 1: "<<std::endl;
    sendNotifySendReady(slot, nbytes);
     std::thread::id this_id = std::this_thread::get_id();
    // std::cout<<"Pair: send 2: "<<this<<" thread: "<<this_id<<std::endl;
    sendUnboundBuffer(std::move(buf), slot, offset, nbytes);
    // std::cout<<"Pair: send 3: "<<std::endl;
    return;
  }

  // Notify peer of this pending send.
  // std::cout<<"Pair: send 4: "<<std::endl;
  localPendingSend_[slot].push_back(std::make_tuple(buf, offset, nbytes));
  // std::cout<<"Pair: send 5: "<<std::endl;
  sendNotifySendReady(slot, nbytes);
  // std::cout<<"Pair: send end"<<std::endl;
}
...
void Pair::recv(
  ...
  // std::cout<<"tcp::mutex before"<<std::endl;
  std::unique_lock<std::mutex> lock(m_);
  // std::cout<<"tcp::mutex after"<<std::endl;
  throwIfException();
// std::cout<<"tcp:: 1"<<std::endl;
  ...
  if (!mutator.shiftRemotePendingSend()) {
    // std::cout<<"tcp:: 3"<<std::endl;
    mutator.pushExpectedSendNotification();
  }
// std::cout<<"tcp:: 4"<<std::endl;
  // Notify peer of this pending recv.
  localPendingRecv_[slot].push_back(std::make_tuple(buf, offset, nbytes));
  // std::cout<<"tcp:: 5"<<std::endl;
  sendNotifyRecvReady(slot, nbytes);
  // std::cout<<"tcp:: 6"<<std::endl;
}
...
// std::cout<<"signalException 3"<<std::endl;
...
// std::cout<<"signalException 4"<<std::endl;
  // Loop through pending recv operations.
  for (auto& it : localPendingRecv_) {
    // std::cout<<"signalException 4.1"<<std::endl;
    for (auto& op : it.second) {
      // std::cout<<"signalException 4.2"<<std::endl;
      NonOwningPtr<UnboundBuffer> buf(std::get<0>(op));
      // std::cout<<"signalException 4.3"<<std::endl;
      if (buf) {
        // std::cout<<"signalException 4.4"<<std::endl;
        buf->signalException(ex);
      }
    }
  }
// std::cout<<"signalException 5"<<std::endl;
  // Store exception_ptr and signal any threads in the async path.
  ex_ = ex;
  // std::cout<<"signalException 6"<<std::endl;
  cv_.notify_all();
// std::cout<<"signalException 7"<<std::endl;
  ...
  changeState(CLOSED);
  // std::cout<<"signalException 8"<<std::endl;
}
